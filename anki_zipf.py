...
def parse_arguments():
    ...
    parser.add_argument(
        "n_words",
        type=int,
        help="Number of most common words to include in the deck"
    )
    parser.add_argument(
        "--output",
...
def get_top_words(language: str, n: int) -> List[str]:
    ...
    try:
        ...
        print("Make sure the language code is correct (e.g., 'en' for English, 'sr' for Serbian)")
        sys.exit(1)


def translate_batch_with_llm(words: List[str], learning_lang: str, target_lang: str, model: str) -> List[Tuple[str, str, str, str]]:
    ...
    try:
        ...
        if json_start != -1 and json_end > json_start:
            ...
            for item in translations:
                if isinstance(item, dict) and 'word' in item and 'translation' in item and 'sentence' in item and 'sentence_translation' in item:
                    result.append((item['word'], item['translation'], item['sentence'], item['sentence_translation']))
        ...
        return [(word, f"[Translation error for {word}]", f"[Sentence error for {word}]", f"[Sentence translation error for {word}]") for word in words]
...
def create_anki_deck(words_data: List[Tuple[str, str, str, str]], learning_lang: str, target_lang: str, output_file: str):
    ...
    my_deck = genanki.Deck(
        deck_id,
        f'{learning_lang.title()} Top Words ({target_lang.title()})'
    )
    ...
    for word, translation, sentence, sentence_translation in words_data:
        note = genanki.Note(
            model=my_model,
            fields=[word, translation, sentence, sentence_translation]
        )
...
def main():
    ...
    for chunk_idx, chunk in enumerate(word_chunks, 1):
        print(f"Processing batch {chunk_idx}/{len(word_chunks)} ({len(chunk)} words)...")
    ...
    create_anki_deck(words_data, args.learning_language, args.translate_to_language, args.output)
